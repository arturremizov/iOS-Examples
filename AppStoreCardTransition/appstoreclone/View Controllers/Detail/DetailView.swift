...
extension DetailView {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if yContentOffset < 0 && scrollView.isTracking {
            ...
            snapshotView.layer.cornerRadius = -yContentOffset > yPositionForDismissal ? 20 : -yContentOffset
            
            // check if reach dismissal position
    ...
    func updateCloseButton(yContentOffset: CGFloat) {
        ...
        if yContentOffset < (cardView?.containerView.frame.maxY ?? 0) - topPadding && cardView?.cardModel.backgroundType == .dark {
